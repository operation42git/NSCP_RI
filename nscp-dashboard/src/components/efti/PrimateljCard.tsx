  ...
  return (
    <div className="flex items-center gap-3 mb-3">
      ...
      <div>
        ...
        <p className="text-[11px] text-[#9aa5b4]">Kontakt osoba (eFTI69)</p>
      </div>
    </div>
  );
}

export default function PrimateljCard({
  party,
  id,
}: {
  party?: TradeParty;
  ...
  if (!party) {
    return (
      ...
      >
        <p className="text-sm text-[#9aa5b4] italic">Podaci o primatelju nisu dostupni.</p>
      </div>
    );
  }
  ...
  const idLabel =
    party.schemeAgencyId === "DE-USt" ? "DE" : party.schemeAgencyId ?? "ID";
          ...
          {formattedAddr ? (
            <>
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {addr?.streetName && <EftiTag label="eFTI74 ulica" />}
                {addr?.buildingNumber && <EftiTag label="eFTI81 k.br." />}
                {addr?.buildingName && <EftiTag label="eFTI78 zgrada" />}
                {addr?.cityName && <EftiTag label="eFTI75 grad" />}
                {addr?.postcode && <EftiTag label="eFTI72 poštanski br." />}
                {addr?.countryCode && <EftiTag label="eFTI76 država" />}
                {addr?.countrySubDivisionName && <EftiTag label="eFTI77 regija" />}
              </div>
            </>
          ) : (
        ...
        <div className="px-5 py-4 border-t border-[#eef0f4] bg-[#fafbff] flex flex-col gap-5">
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <FieldPair label="Porezni ID" eftiId="eFTI1458" value={tax?.id} />
              <FieldPair label="Shema" eftiId="eFTI1459" value={tax?.schemeAgencyId} />
            </div>
          </DetailGroup>
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <FieldPair label="Ime" eftiId="eFTI1156" value={contact?.givenName} />
              <FieldPair label="Prezime" eftiId="eFTI1158" value={contact?.familyName} />
            </div>
          </DetailGroup>
