...
import { ECMRData, Party, Location, ConsignmentItem, PostalAddress, ServiceCharge, ADRData, ADRItem } from '../models/ecmr.model';

@Injectable({
  providedIn: 'root'
})
export class ECMRParserService {

  private readonly NS = 'http://efti.eu/v1/consignment/common';

  parseXML(xmlString: string): ECMRData {
    ...
    const getText = (element: Element | null, localName: string): string | undefined => {
      ...
      let nodes = element.getElementsByTagNameNS(this.NS, localName);
    ...
    if (associatedDocument) {
      ...
      if (issueDate || issueLoc) {
        ...
        if (issueLoc) {
          ...
          const locAddress = locAddressObj ? this.formatAddress(locAddressObj) : '';
          if (locName) {
    ...
    // Fields 22-24: Signatures (reuse consignor, carrier, consignee variables declared earlier)
    if (consignor) {
    ...
    if (firstServiceCharge) {
      ...
      if (!ecmr.tariff27) {
        ...
        if (amount) {
          ecmr.tariff27 = this.formatCurrency(
    ...
    ecmr.adrData = this.parseADRData(root, getElement, getElements, getText, getAttribute, formatParty);

    return ecmr;
  }

  formatAddress(address: PostalAddress | undefined): string {
    if (!address) return '';
    const parts: string[] = [];
  ...
  formatParty(party: Party | undefined): string {
    if (!party) return '';
    const address = this.formatAddress(party.postalAddress);
    if (party.name) {
  ...
  formatLocation(location: Location | undefined): string {
    if (!location) return '';
    const address = this.formatAddress(location.postalAddress);
    if (location.name) {
  ...
  private parseADRData(
    ...
    // Freight Forwarder
    const freightForwarder = getElement(root, 'freightForwarder');
    if (freightForwarder) {
      adrData.freightForwarder = formatParty(freightForwarder);
    }
